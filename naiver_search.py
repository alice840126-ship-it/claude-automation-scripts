...
사용법: python3 naaver_search.py '검색어' [검색타입]
...
def naver_search(query, search_type='webkr', display=10, start=1, sort='sim'):
    """
    네이버 검색 API 호출

    Args:
        query: 검색어
        search_type: 검색 타입 (webkr, blog, cafe, news, kin, shop, doc)
    ...
    if not NAVER_CLIENT_ID or not NAVER_CLIENT_SECRET:
        print("❌ NAVER_CLIENT_ID or NAVER_CLIENT_SECRET not found in .env file")
        return None
...
def format_result(result, search_type, query):
    """검색 결과를 보기 좋게 포맷팅"""
    ...
    output = []
    output.append(f"🔍 네이버 {search_type.upper()} 검색: '{query}'")
    output.append(f"검색 결과 총 {total:,}건 중 {len(items)}건\n")
    ...
    for i, item in enumerate(items[:5], 1):  # 상위 5개만
        ...
        if search_type in ['blog', 'cafe']:
            ...
            output.append(f"내용: {description[:150]}...")
...
def main():
    if len(sys.argv) < 2:
        print("사용법: python3 naiver_search.py '검색어' [검색타입]")
    ...
    if search_type not in valid_types:
        print(f"❌ 유효하지 않은 검색타입 '{search_type}'")
    ...
    if result:
        print(format_result(result, search_type, query))
        print(f"\n✅ 검색 완료!")
    else:
        print("❌ 검색 실패")
